...
users {
        ...
        String email
        String password
        String role "DONOR, ASSOC, ADMIN"
        String name
        Object profile "Sub-doc: donorType, status, etc."
        Number solidarityPoints
        ObjectId redeemedRewards FK "Array[ref: rewards]"
    }

    donations {
        ObjectId _id PK
        ObjectId donorId FK "ref: users"
        ObjectId associationId FK "ref: users"
        String status
        String type
        String quantity
        Object pickupLocation "Sub-doc: address, geo"
        Object evaluation "Sub-doc: rating, comment"
    }

    reports {
        ObjectId _id PK
        ObjectId reporterId FK "ref: users"
        ObjectId reportedUserId FK "ref: users"
        ObjectId donationId FK "ref: donations"
        String type "MALFUNCTION, USER_BEHAVIOR"
        String description
    ...
    users ||--o{ donations : "crea (donorId)"
    users ||--o{ donations : "accetta (associationId)"
    users ||--o{ inventoryItems : "possiede (associationID)"
    users ||--o{ reports : "crea (reporterId)"
    users ||--o{ reports : "è segnalato in (reportedUserId)"
    donations ||--o{ reports : "è menzionata in (donationId)"
    users }o--o{ rewards : "riscatta"
```
